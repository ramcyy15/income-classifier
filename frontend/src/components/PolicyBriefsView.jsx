...
import { FileText, Building2, Sparkles, CheckCircle2, ChevronRight, ShieldAlert, RefreshCw, Cpu } from 'lucide-react';
...
export default function PolicyBriefsView({ barangays, selectedBarangay, onSelectBarangay, onOpenSimulation }) {
  ...
  const [liveBriefs, setLiveBriefs] = useState({});
  const [aiError, setAiError] = useState(null);

  const currentBrief = (activeBrgy?.name && liveBriefs[activeBrgy.name]) || activeBrgy?.policy_brief || {};
  const programs = currentBrief.programs || [];
  const suggestions = currentBrief.slider_suggestion || {};
  const isGeneratedLive = activeBrgy?.name && Boolean(liveBriefs[activeBrgy.name]);

  const handleGenerateLive = async () => {
    if (!activeBrgy?.name || loadingAi) return;
    setLoadingAi(true);
    setAiError(null);
    try {
      const res = await generateLivePolicyBrief(activeBrgy.name);
      if (res.policy_brief) {
        setLiveBriefs((prev) => ({
          ...prev,
          [activeBrgy.name]: res.policy_brief,
        }));
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner with Selector & Live Action */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-3.5">
          <img
            src={kalingaLogo}
            alt="KalingaBot Logo"
            className="w-12 h-12 rounded-xl object-contain bg-slate-50 border border-slate-100 shadow-sm p-0.5 flex-shrink-0"
          />
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">
              Evidence-based policy synthesis combining 4Ps graduation data, household demographics, and ML predictions.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            disabled={loadingAi}
            className="py-2 px-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1.5 cursor-pointer"
          >
            {loadingAi ? (
      ...
      {aiError && (
        <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700 flex items-center justify-between">
          <span>{aiError}</span>
          <button onClick={() => setAiError(null)} className="font-bold underline text-red-800 ml-2">Dismiss</button>
        </div>
      )}
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
            ...
            <p className="text-xs text-slate-600 leading-relaxed font-medium">
              {currentBrief.summary || 'No policy brief found for this barangay.'}
            </p>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-semibold text-slate-400 uppercase mb-2">
                    ...
                    {p.sector && (
                      <>
                        <span>&middot;</span>
                        <span className="text-indigo-600 bg-indigo-50/80 px-2 py-0.5 rounded-md font-bold text-[10px]">
                          {p.sector}
                        </span>
                      </>
                    )}
